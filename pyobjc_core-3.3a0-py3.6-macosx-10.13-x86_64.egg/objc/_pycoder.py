...
if sys.version_info[0] == 2:  # pragma: no 3.x cover
    ...
    encode_dispatch[int] = save_long

else: # pragma: no 2.x cover
    def save_int(coder, obj):
        if coder.allowsKeyedCoding():
            coder.encodeInt_forKey_(kOP_LONG, kKIND)
            coder.encodeObject_forKey_(str(repr(obj)), kVALUE)
        else:
            coder.encodeValueOfObjCType_at_(objc._C_INT, kOP_LONG)
            coder.encodeObject_(str(repr(obj)))
    encode_dispatch[int] = save_int
...
def save_global(coder, obj, name=None):
    ...
    if coder.allowsKeyedCoding():
        if code:
            ...
            coder.encodeObject_forKey_(str(module_name), kMODULE)
            coder.encodeObject_forKey_(str(name), kNAME)
        ...
        if code:
            ...
            coder.encodeObject_(str(module_name))
            coder.encodeObject_(str(name))
...
def load_reduce(coder, setValue):
    ...
    if dictitems:
        for k, v in list(dictitems.items()):
            value[k] = v
