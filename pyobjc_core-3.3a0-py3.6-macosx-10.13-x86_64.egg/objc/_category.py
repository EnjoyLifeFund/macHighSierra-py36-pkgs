...
class _CategoryMeta(type):
    ...
    def __new__(cls, name, bases, methods):
        ...
        m = [ x[1] for x in list(methods.items()) if x[0] not in cls._IGNORENAMES  and isinstance(x[1], (FunctionType, MethodType, selector, classmethod))]
        vars = [ x for x in list(methods.items()) if x[0] not in cls._IGNORENAMES  and not isinstance(x[1], (FunctionType, MethodType, selector, classmethod))]
        for k, v in vars:
